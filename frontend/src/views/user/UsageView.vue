...
const hasImageOutputTokens = (row: Pick<UsageLog, 'image_output_tokens'> | null | undefined): boolean => {
  return (row?.image_output_tokens ?? 0) > 0
}

const textOutputTokens = (row: Pick<UsageLog, 'output_tokens' | 'image_output_tokens'> | null | undefined): number => {
  const textTokens = (row?.output_tokens ?? 0) - (row?.image_output_tokens ?? 0)
  return textTokens > 0 ? textTokens : 0
}

const hasImageOutputCost = (row: Pick<UsageLog, 'image_output_cost'> | null | undefined): boolean => {
