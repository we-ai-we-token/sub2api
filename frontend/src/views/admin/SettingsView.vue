    ...
    <div class="mx-auto max-w-6xl space-y-6">
      ...
      <form v-else @submit.prevent="saveSettings" class="space-y-6" novalidate>
        ...
        <div class="card">
          <div class="border-b border-gray-100 px-6 py-4 dark:border-dark-700">
            <h2 class="text-lg font-semibold text-gray-900 dark:text-white">
              {{ t('admin.settings.features.synthCacheHidden.title') }}
            </h2>
            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
              {{ t('admin.settings.features.synthCacheHidden.description') }}
            </p>
          </div>
          <div class="space-y-5 p-6">
            <div class="flex items-center justify-between">
              <div>
                <label class="text-sm font-medium text-gray-700 dark:text-gray-300">
                  {{ t('admin.settings.features.synthCacheHidden.enabled') }}
                </label>
                <p class="mt-0.5 text-xs text-gray-500 dark:text-gray-400">
                  {{ t('admin.settings.features.synthCacheHidden.enabledHint') }}
                </p>
              </div>
              <Toggle v-model="form.openai_synth_cache_hidden_enabled" />
            </div>
          </div>
        </div>
...
const form = reactive<SettingsForm>({
  ...
  openai_synth_cache_hidden_enabled: false,
...
async function saveSettings() {
  ...
  try {
    ...
    const payload: UpdateSettingsRequest = {
      ...
      // 隐藏/扣减自动注入的 Codex instructions 缓存 token
      openai_synth_cache_hidden_enabled: form.openai_synth_cache_hidden_enabled,
      // Model Plaza feature switches + description
