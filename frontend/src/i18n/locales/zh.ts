export default {
  ...
  modelPlaza: {
    ...
    exclusive: '专属',
    perRequest: '每次',
    imagePerRequestTag: '图片按次',
    exchangeHint: '本平台 1 人民币 = 1 美元。',
  ...
  admin: {
    ...
    groups: {
      ...
      imagePricing: {
        ...
        qualityBilling: 'OpenAI 按返回质量计费',
        imageMultiplier: '生图独立倍率',
        modeHint: '默认按 1K/2K/4K 分辨率计费；OpenAI 可改为按 usage.quality 的 low/medium/high 计费，缺失 quality 时按 low。',
    ...
    riskControl: {
      ...
      blockedKeywords: '关键词风控规则',
      blockedKeywordsPlaceholder: '每行一条规则，例如：\n敏感词1\nre:token-[a-z0-9]{8}\n账号 && 密码',
      blockedKeywordsDescription: '在 AI 模型审核之前先做关键词匹配。每行一条规则，"re:" 前缀表示正则；同一行用 "&&" 连接多段表示 AND（必须全部命中）。行间为 OR。命中后跟随上方"运行模式"：PreBlock 拦截、Observe 仅记录、Off 不启用。',
      blockedKeywordsPreBlockHint: '关键词风控规则会在 AI 模型审核之前先执行。',
      blockedKeywordsModeWarning: '当前为「{mode}」模式，关键词风控规则只记录命中或不启用；请按上方运行模式确认预期行为。',
      blockedKeywordCount: '已配置 {count} 条规则',
      blockedKeywordsLimit: '最多保存 {max} 条规则，单条长度不超过 200 个字符；重复项会自动去重。',
      keywordBlockingMode: '审计策略',
      keywordModeKeywordAndApi: '关键词 + API',
      keywordModeKeywordAndApiDesc: '先执行关键词风控规则；未命中时再调用上游审计接口。',
      keywordModeKeywordOnly: '仅关键词',
      keywordModeKeywordOnlyDesc: '只用关键词风控规则判断，未命中即放行，不调用上游审计接口，可显著降低 API 用量。',
      keywordModeKeywordOnlyNotice: '当前为「仅关键词」策略：未命中关键词风控规则的请求将直接放行，不调用上游审计接口。',
      keywordModeApiOnly: '仅 API',
      keywordModeApiOnlyDesc: '只调用上游审计接口判断，本页的关键词风控规则将不会生效。',
      keywordModeApiOnlyNotice: '当前为「仅 API」策略：关键词风控规则不会生效，请求会全部交给上游审计接口判断。',
      overview: {
        status: '运行状态',
        enabled: '已启用',
        disabled: '未启用',
