export default {
  ...
  usage: {
    ...
    imageBillingTier: 'Billing tier',
    imageQuality: 'Returned quality',
  ...
  modelPlaza: {
    ...
    exclusive: 'Exclusive',
    perRequest: 'Per request',
    imagePerRequestTag: 'Image / per-request',
    exchangeHint: 'On this platform, CNY 1 = USD 1.',
  ...
  admin: {
    ...
    groups: {
      ...
      imagePricing: {
        ...
        qualityBilling: 'OpenAI bill by returned quality',
        imageMultiplier: 'Image multiplier',
        modeHint: 'Default image billing uses 1K/2K/4K size tiers. OpenAI can bill by usage.quality low/medium/high; missing quality bills as low.',
    ...
    riskControl: {
      ...
      tabs: {
        ...
        response: 'Hit Notice',
        riskThresholds: 'Risk Thresholds',
        keywords: 'Keyword Risk Rules',
        retention: 'Retention',
      },
      blockedKeywords: 'Keyword risk rules',
      blockedKeywordsPlaceholder: 'One rule per line, e.g.:\nbadword1\nre:token-[a-z0-9]{8}\naccount && password',
      blockedKeywordsDescription: 'Keyword matching runs before AI model moderation. One rule per line; "re:" means regex; join segments with "&&" on the same line for AND (all must match). Lines are OR. After a hit, the runtime mode applies: PreBlock blocks, Observe records only, Off disables.',
      blockedKeywordsPreBlockHint: 'Keyword risk rules run before AI model moderation.',
      blockedKeywordsModeWarning: 'Current mode is "{mode}". Keyword risk rules will only record hits or stay disabled; confirm the runtime mode above matches your intent.',
      blockedKeywordCount: '{count} rules configured',
      ...
      keywordModeKeywordAndApiDesc: 'Run keyword risk rules first; misses fall through to the upstream moderation API.',
      keywordModeKeywordOnly: 'Keyword only',
      keywordModeKeywordOnlyDesc: 'Decide using keyword risk rules only; misses are allowed without calling the API, saving upstream cost.',
      keywordModeKeywordOnlyNotice: 'Keyword-only strategy: requests that do not match any keyword risk rule are allowed without calling the upstream moderation API.',
      keywordModeApiOnly: 'API only',
      keywordModeApiOnlyDesc: 'Use the upstream moderation API only; the keyword risk rules configured here are not consulted.',
      keywordModeApiOnlyNotice: 'API-only strategy: keyword risk rules are not consulted; all requests go through the upstream moderation API.',
      overview: {
        status: 'Status',
        enabled: 'Enabled',
        disabled: 'Disabled',
