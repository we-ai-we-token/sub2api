...
// 二开：只要有图片输入单价就展示，不区分计费模式。
// 没有「这是生图模型」的标记位，有没有正的 image_input_price 本身就是判定
// （与 components/channels/SupportedModelChip.vue:101 的既有做法一致）。
describe('PlazaModelPricingTable 图片输入单价', () => {
  const imageModel = (imageInputPrice: number | null) =>
    tokenModel({
      name: 'gpt-image-2.5-flare',
      platform: 'openai',
      pricing: { ...tokenModel().pricing!, image_input_price: imageInputPrice }
    })

  it('有图片输入 token 单价时按 $/1M 展示', () => {
    const text = mountTable([imageModel(5e-6)], 1).text()
    expect(text).toContain('modelPlaza.table.imageInputShort')
    expect(text).toContain('$5.00')
  })

  it('跟随分组/用户倍率', () => {
    const text = mountTable([imageModel(5e-6)], 2).text()
  ...
  it('按次计费的模型同样展示', () => {
    ...
    const text = mountTable([m], 1).text()
    expect(text).toContain('modelPlaza.table.imageInputShort')
    expect(text).toContain('$5.00')
  })
  ...
  it('没有图片输入单价时不展示该行', () => {
    expect(mountTable([imageModel(null)], 1).text()).not.toContain('modelPlaza.table.imageInputShort')
    expect(mountTable([imageModel(0)], 1).text()).not.toContain('modelPlaza.table.imageInputShort')
  })
})
