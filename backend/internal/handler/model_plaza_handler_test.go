...
func TestModelPlaza_Unauthenticated401(t *testing.T) {
	...
	h := &ModelPlazaHandler{} // nil services — 401 路径不触达依赖
...
func TestExpandWildcardModels_PrefixMatch(t *testing.T) {
	...
	got := expandWildcardModels([]service.AvailableChannel{ch}, group, accountModels)
...
func TestExpandWildcardModels_PlatformMismatch(t *testing.T) {
	...
	got := expandWildcardModels([]service.AvailableChannel{ch}, group,
...
func TestExpandWildcardModels_NoAccountModels(t *testing.T) {
	...
	require.Empty(t, expandWildcardModels([]service.AvailableChannel{ch}, group, nil))
}
...
func TestResolveImageTier_ChannelFlatBeatsGroup(t *testing.T) {
	...
	for _, tier := range []string{"1K", "2K", "4K"} {
		got := resolveImageTier(p, group, tier)
...
func TestResolveImageTier_GroupFallbackWhenNoChannelPrice(t *testing.T) {
	...
	got := resolveImageTier(p, group, "2K")
	require.NotNil(t, got)
...
func TestResolveImageTier_TierIntervalBeatsFlat(t *testing.T) {
	...
	require.InDelta(t, 0.3, *resolveImageTier(p, group, "4K"), 1e-9) // 区间命中
	require.InDelta(t, 0.1, *resolveImageTier(p, group, "1K"), 1e-9) // 回落 flat
}

// 端到端：buildModelPlazaModels 把展开出的具体模型作为按次行返回（原价，未折算）。
func TestBuildModelPlazaModels_WildcardExpandedAsPerRequest(t *testing.T) {
	...
	models := buildModelPlazaModels([]service.AvailableChannel{ch}, group, accountModels, nil)
...
func TestBuildModelPlazaModels_TokenModelsFromBuiltinPricing(t *testing.T) {
	...
	models := buildModelPlazaModels(nil, group, []string{"gpt-5.5", "gpt-5.4"}, pricingFor)

	byName := map[string]modelPlazaModel{}
...
func TestBuildModelPlazaModels_FourImageModels(t *testing.T) {
	...
	models := buildModelPlazaModels([]service.AvailableChannel{ch}, group, accountModels, pricingFor)
