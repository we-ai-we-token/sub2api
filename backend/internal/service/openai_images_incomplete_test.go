...
func TestImagesOAuthNonStreaming_CompletedNoImageTriggersSameAccountRetry(t *testing.T) {
	...
	// 本地保留 sentinel 重试机制（errOpenAIImagesEmptyOutputRetryable）替代上游的
	// UpstreamFailoverError：completed 但无图（且无文字拒绝）时返回可重试 sentinel，由
	// 调用方据尝试次数优先同账号重试，用尽后再 failover 换账号。第 5 参传 true 表示本次
	// 尝试仍在可重试窗口内。
	_, _, _, err := svc.handleOpenAIImagesOAuthNonStreamingResponse(resp, c, "b64_json", "gpt-image-2", true)

	if err == nil {
		t.Fatal("completed-but-no-image should return an error")
	}
	if !errors.Is(err, errOpenAIImagesEmptyOutputRetryable) {
		t.Fatalf("soft-failure should return the retryable empty-output sentinel for same-account retry, got %T: %v", err, err)
	}
	// 不应被误判为不可重试的内容策略错误（那是 refusal 场景）。
	var imgErr *OpenAIImagesUpstreamError
	if errors.As(err, &imgErr) {
		t.Fatalf("empty output must not be a non-retryable content error, got %v", imgErr)
	}
}
...
func TestImagesOAuthStreaming_TextFallbackReturnsCapabilityError(t *testing.T) {
	...
	svc := &OpenAIGatewayService{}
	_, _, _, _, err := svc.handleOpenAIImagesOAuthStreamingResponse(resp, c, time.Now(), "b64_json", "image_generation", "gpt-image-2", false)
