...
func TestOpenAIGatewayServiceForwardImages_OAuthGroupResponsesFlagUsesResponsesEndpoint(t *testing.T) {
	...
	//
	// 模型用 gpt-image-1：v0.2.5 起 usesCodexDirectImages 会把 gpt-image-1 等型号
	// 直接打到 codex images 端点，用它就测不到 Responses 分支了（上游对自己的
	// Responses 用例做了同样的替换）。
	gin.SetMode(gin.TestMode)
	body := []byte(`{"model":"gpt-image-1","prompt":"draw a cat","size":"1024x1024","quality":"high","n":1}`)
...
func TestOpenAIImagesOAuthBodyReadTransportErrorFailover(t *testing.T) {
	...
	_, _, _, readErr := svc.handleOpenAIImagesOAuthNonStreamingResponse(resp, c, "b64_json", "gpt-image-2", false)
