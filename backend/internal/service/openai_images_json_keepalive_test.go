...
func TestOpenAIImagesJSONKeepalive_KeepsOAuthNonStreamResponseValid(t *testing.T) {
	...
	_, imageCount, _, err := svc.handleOpenAIImagesOAuthNonStreamingResponse(resp, c, "b64_json", "gpt-image-2", false)
	stop()
...
func TestOpenAIImagesJSONKeepalive_HeartbeatBeforeForwardStillFailsOver(t *testing.T) {
	...
	c, _ := gin.CreateTestContext(rec)
	c.Request = req
	// 本地二开：生图默认走上游 Responses 链路（Group.ImageUseResponsesAPI 默认 true），
	// 该链路才能解析 SSE error 事件并透传上游错误；显式注入 group 复现生产默认行为。
