...
// 图片 quality 计费档位（OpenAI 生图按 quality 计费时使用）。
const (
	ImageQualityLow    = "low"
...
// ImageQualityPriceConfig 按 quality 计费的分组价格配置。
// 任一档为 nil 表示该档使用模型默认图片价。
type ImageQualityPriceConfig struct {
	PriceLow    *float64
	PriceMedium *float64
	PriceHigh   *float64
}

// NormalizeImageQualityOrDefault 归一化 OpenAI 返回的 quality；
// 非 low/medium/high（含空串、auto 等）一律按 medium 计费。
func NormalizeImageQualityOrDefault(quality string) string {
...
// CalculateImageCostByQuality 按 quality（low/medium/high）计算图片生成费用。
// quality 以 OpenAI 响应返回值为准，未返回时归一化为 medium。
func (s *BillingService) CalculateImageCostByQuality(model string, quality string, imageCount int, qualityConfig *ImageQualityPriceConfig, rateMultiplier float64) *CostBreakdown {
	if imageCount <= 0 {
		return &CostBreakdown{}
	}
	quality = NormalizeImageQualityOrDefault(quality)

	unitPrice := s.getImageQualityUnitPrice(model, quality, qualityConfig)
	totalCost := unitPrice * float64(imageCount)
...
// getImageQualityUnitPrice 获取按 quality 的图片单价；
// 分组未配置该档价格时回退到模型默认图片价。
func (s *BillingService) getImageQualityUnitPrice(model string, quality string, qualityConfig *ImageQualityPriceConfig) float64 {
	if qualityConfig != nil {
		switch quality {
		case ImageQualityLow:
			if qualityConfig.PriceLow != nil {
				return *qualityConfig.PriceLow
			}
		case ImageQualityMedium:
			if qualityConfig.PriceMedium != nil {
				return *qualityConfig.PriceMedium
			}
		case ImageQualityHigh:
			if qualityConfig.PriceHigh != nil {
				return *qualityConfig.PriceHigh
			}
		}
	}

	// 回退到模型默认图片价（与 1K 同基准，不叠加尺寸倍率）。
	return s.getDefaultImagePrice(model, "1K")
}
