...
func newOpenAIImagesEmptyOutputFailoverError() *UpstreamFailoverError {
	return &UpstreamFailoverError{
		...
		ResponseBody: []byte(`{"error":{"message":"openai images upstream returned no image output","type":"upstream_error","code":"upstream_error"}}`),
	}
}

func IsOpenAIImagesEmptyOutputFailoverError(err *UpstreamFailoverError) bool {
	return err != nil && bytes.Contains(err.ResponseBody, []byte(openAIImagesEmptyOutputFailoverMessage))
}
...
func (s *OpenAIGatewayService) handleOpenAIImagesOAuthNonStreamingOutput(
	resp *http.Response,
	c *gin.Context,
	responseFormat string,
	...
	if len(results) == 0 {
		...
		// 上游文字兜底（v0.1.183 起）：模型未出图但输出了文字。
		// 内容审核拒绝（安全/审核关键词）→ 400 content_policy_violation，换账号/重试均无
		// 效，直接透传给客户端；其余纯文字回应 → 502 image_generation_unavailable，属于
		// 上游能力性失败，交由 handleOpenAIImagesOAuthResponseError 冷却账号并 failover。
		// 此判定需置于 foundFinal 之前，因文字兜底通常也带 final 事件，但内容层决策应优先。
...
func (s *OpenAIGatewayService) handleOpenAIImagesOAuthNonStreamingResponse(
	...
	if err != nil {
		// 只有既非本地可重试（特殊瞬态）、也非上游 5xx 服务端错误时，才把错误直接写给客户端。
		// 写响应会关闭 failover 窗口（responseWritten），5xx 需留给上层换账号重试。
		if upstreamErr, ok := err.(*OpenAIImagesUpstreamError); ok &&
			!IsRetryableOpenAIImagesUpstreamError(upstreamErr) &&
			!IsOpenAIImagesRetryableUpstreamError(upstreamErr) &&
			!c.Writer.Written() {
			writeOpenAIImagesUpstreamErrorResponse(c, upstreamErr)
		}
		if output != nil {
			return output.Usage, len(output.ImageResults), output.ImageSizes, err
		}
	...
	responseBody, err := buildOpenAIImagesAPIResponse(output.ImageResults, output.CreatedAt, output.UsageRaw, output.FirstMeta, responseFormat)
	if err != nil {
		return OpenAIUsage{}, 0, nil, err
	}
	...
	return output.Usage, len(output.ImageResults), output.ImageSizes, nil
}
...
func (s *OpenAIGatewayService) handleOpenAIImagesOAuthStreamingResponse(
	...
	processData := func(dataBytes []byte) {
		...
		case "response.completed":
			...
			if len(finalResults) == 0 {
				// 上游文字兜底（v0.1.183 起）：模型改用文字回应而未出图。内容审核拒绝 →
				// 400 直接写给客户端；其余纯文字 → 502 image_generation_unavailable，
				// 交由 handleOpenAIImagesOAuthResponseError 冷却账号并 failover。
				...
				// 保留本地线上行为：真空响应（既无图也无文字）时返回可重试 sentinel，
		...
		case "error", "response.failed":
			if upstreamErr := openAIImagesUpstreamErrorFromSSEPayload(dataBytes); upstreamErr != nil {
				retryable := IsRetryableOpenAIImagesUpstreamError(upstreamErr)
...
func (s *OpenAIGatewayService) forwardOpenAIImagesOAuthOnce(
	...
	const maxEmptyOutputAttempts = 1
	for attempt := 1; attempt <= maxEmptyOutputAttempts; attempt++ {
		...
		output.UpstreamModel = requestModel
		output.RequestID = resp.Header.Get("x-request-id")
		return output, nil
	}
	return nil, newOpenAIImagesEmptyOutputFailoverError()
}

func (s *OpenAIGatewayService) forwardOpenAIImagesOAuthStreaming(
	ctx context.Context,
	c *gin.Context,
	account *Account,
	parsed *OpenAIImagesRequest,
